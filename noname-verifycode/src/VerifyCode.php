...
class VerifyCode {
     ...
     */
    public function __construct($type = self::TYPE_MIX, $num = 5, $size = 16, $height = NULL, $width = NULL, $distortion = false, $border = false, $noise_point_num = 30, $noise_line_num = 3, $bg_img = "", $font_family = "", array $font_color = [])
    {
        $this->type = $type;
        $this->text_num = $num;
        $this->font_size = $size;
        ...
        //字体自定义
        $this->font_family = $font_family ?: __DIR__ . "/font/default.ttf";
        $this->run();
    }
    /**获取图片宽度默认值
     *
    ...
    protected function textMergeImg()
    {
        ...
        if (self::TYPE_NUM === $this->type) {
            $text_arr[] = '=';
            $text_arr[] = '?';
        }
        foreach ($text_arr as $i => $text) {
            if (self::TYPE_IMG === $this->type) {
            ...
            } if(self::TYPE_MIX === $this->type) {
                $x = 5 + $i * floor($this->font_size * 1.3);
                imagettftext($this->image, $this->font_size, mt_rand(-1, 1) * mt_rand(1, 20), $x, floor($this->img_height * 0.75), $this->font_color, $this->font_family, $text);
            }else {
                if ($i == 1) {
                    $x = 20 + $i * floor($this->font_size * 1.3);
                } else {
                    $x = 10 + $i * floor($this->font_size * 1.3);
                }
                imagettftext($this->image, $this->font_size, mt_rand(-1, 1) * mt_rand(1, 20), $x, floor($this->img_height * 0.75), $this->font_color, $this->font_family, $text);
            }
        }
